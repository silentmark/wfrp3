class WFRP_Audio 
{
    static PlayContextAudio(context)
    {
        this.MatchContextAudio(context).then(sound => {
            console.log(`wfrp3 | Playing Sound: ${sound.file}`)
    ...
    {
      if (!game.settings.get("wfrp3", "soundEffects") || !context)
        return {}
        
      try {
        let files = ""
        let file, group;
        await FilePicker.browse("user", `systems/wfrp3/sounds`).then(resp => {
          files = resp.files
        })
      ...
      {
        console.log("wfrp3 | Sound Context Error: " + e)
      }
    }
}