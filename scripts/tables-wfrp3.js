...
class WFRP_Tables
{
  ...
  static formatChatRoll(table, options = {}, column = null)
  {
    ...
    switch (table)
    {
      ...
      case "career":
        return `<b>Random Career - ${wfrp3.species[column]}</b><br> <a class = "item-lookup">${result.name}</a> <br> <b>Roll:</b> ${result.roll}`;
      case "eyes":
      case "hair":
        return `<b>${this[table].name} - ${wfrp3.species[column]}</b><br>${result.name}<br><b>Roll:</b> ${eval(result.roll)}`
