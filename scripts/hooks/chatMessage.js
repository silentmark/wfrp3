...
Hooks.on("chatMessage", (html, content, msg) => {
     ...
     * @returns {{amount: string, option: wfrp3.creditOptions}}
     */
    function extractAmountAndOptionFromCommandLine(commands) {
        ...
        if (isAnAmount) {
            ...
            optionInCommandLine = wfrp3.creditOptions.SPLIT;
        } else {
     ...
     * @returns {wfrp3.creditOptions} an option
     */
    function getOption(optionInCommandLine) {
      return (typeof optionInCommandLine == "undefined") ? wfrp3.creditOptions.SPLIT : optionInCommandLine;
    }    
...
// Roll on a table
    ...
    // Lookup a condition
        ...
        let closest = WFRP_Utility.matchClosest(wfrp3.conditions, conditionInput);
        let description = wfrp3.conditionDescriptions[closest];
        let name = wfrp3.conditions[closest];
    ...
    // Availability test
        ...
        Marketwfrp3.testForAvailability({settlement, rarity, modifier});
        return false;
    }
    // Pay commands
    else if (command === "/pay") {
    ...
    // Credit commands
        ...
        if (game.user.isGM) {
            Marketwfrp3.generateCreditCard(amount, option);
        } else {
    ...
    //Help commands
        ...
        renderTemplate("systems/wfrp3/templates/chat/chat-help-command.html", {
