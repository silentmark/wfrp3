...
class WFRP_Utility
{
  ...
  static _spellDescription(spell)
  {
    ...
    else if (wfrp3.loreEffect[spell.data.lore.value])
      description += "\n\n <b>Lore:</b> " + wfrp3.loreEffect[spell.data.lore.value];
    return description;
  }
  ...
  {
    let characteristics = {};
    let characteristicFormulae = wfrp3.speciesCharacteristics[species];
    try
    {
      if (!characteristicFormulae) // If input species was not a valid key, try finding it as a value
        characteristicFormulae = wfrp3.speciesCharacteristics[this.findKey(species, wfrp3.species)]
    }
    catch (error)
    {
      ui.notifications.info("Could not find species " + species)
      console.log("wfrp3 | Could not find species " + species + ": " + error);
      throw error
    }

    for (let char in wfrp3.characteristics)
    {
      if (average)
      {
  ...
  {
    let move = wfrp3.speciesMovement[species];
    if (!move) // If input species was not a valid key, try finding it as a value
      move = wfrp3.speciesMovement[this.findKey(species, wfrp3.species)]
    return move;
  }
  ...
  // Used to sort arrays based on string value (used in organizing skills to be alphabetical - see Actorwfrp3.prepareItems())
  ...
  static _calculateAdvCost(currentAdvances, type)
  {
    ...
    if (index >= wfrp3.xpCost[type].length)
      return wfrp3.xpCost[wfrp3.xpCost.length - 1];
    return wfrp3.xpCost[type][index];
  }
  ...
  static displayStatus(tokenId, round = undefined)
  {
    ...
    chatOptions["template"] = "systems/wfrp3/templates/chat/combat-status.html"
  ...
  static displayRoundSummary(combat)
  {
    ...
    if (["gmroll", "blindroll"].includes(chatOptions.rollMode)) chatOptions["whisper"] = ChatMessage.getWhisperIDs("GM");
    if (chatOptions.rollMode === "blindroll") chatOptions["blind"] = true;
    chatOptions["template"] = "systems/wfrp3/templates/chat/round-summary.html"
  ...
  static parseConditions(effectList)
  {
    ...
    {
      let displayValue = (wfrp3.conditions[c])
  ...
  {
    let symkey = WFRP_Utility.findKey(symptom.split("(")[0].trim(), wfrp3.symptoms)
    let content = `<b>${symptom}</b>: ${wfrp3.symptomDescriptions[symkey]}`;
    let chatOptions = {
      user: game.user._id,
      rollMode: game.settings.get("core", "rollMode"),
      content: content
    ...
    if (game.user.isGM)
    {
      content = `<b>${symptom} Treatment</b>: ${wfrp3.symptomTreatment[symkey]}`;
      chatOptions = {
        user: game.user._id,
        rollMode: game.settings.get("core", "rollMode"),
        content: content
  ...
  static postProperty(property)
  {
    let properties = mergeObject(WFRP_Utility.qualityList(), WFRP_Utility.flawList()),
      propertyDescr = Object.assign(duplicate(wfrp3.qualityDescriptions), wfrp3.flawDescriptions),
      propertyKey;
  ...
  {
    let moneyItems = []
    const trappings = game.packs.find(p => p.collection == "wfrp3.trappings")
    let trappingsIndex = [];
  ...
  static handleTableClick(event)
  {
    ...
    {
      renderTemplate('systems/wfrp3/templates/chat/table-dialog.html').then(html =>
      {
        new Dialog(
        {
  ...
  static handleConditionClick(event)
  {
    ...
    cond = cond.split(" ")[0]
    let condkey = WFRP_Utility.findKey(cond, wfrp3.conditions);
    let condDescr = wfrp3.conditionDescriptions[condkey];
  ...
  static handlePayClick(event)
  {
    ...
    if (game.user.isGM)
      Marketwfrp3.generatePayCard(payString);
  }
  ...
  {
    let morrsliebActive = canvas.scene.getFlag("wfrp3", "morrslieb")
    morrsliebActive = !morrsliebActive
    await canvas.scene.setFlag("wfrp3", "morrslieb", morrsliebActive)
