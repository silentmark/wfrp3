 ...
 * 
 * Targeted flow:
 * Every roll (see roll overrides, Actorwfrp3) checks to see if a target is selected, if so, handleOpposed is called. See this function for details
    ...
    try
    {
      ...
      if (attackerSL > defenderSL || (attackerSL === defenderSL && attacker.testResult.target > defender.testResult.target))
      {
        ...
        if (!isNaN(opposeResult.attackerTestResult.damage))
        {
          ...
          let sizeDiff = wfrp3.actorSizeNums[opposeResult.attackerTestResult.size] - wfrp3.actorSizeNums[opposeResult.defenderTestResult.size]
      ...
      Hooks.call("wfrp3:opposedTestResult", opposeResult)
      ...
      if (options.target)
      {
        ...
        renderTemplate("systems/wfrp3/templates/chat/opposed-result.html", opposeResult).then(html =>
        {
          let chatOptions = {
            user: game.user._id,
            content: html,
