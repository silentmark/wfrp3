/** Class for the wfrp3 Item Browser that collects all items in the world and compendia and
...
*/
class Browserwfrp3 extends Application
{
  constructor(app)
  {
    super(app)
  ...
  static get defaultOptions()
  {
    ...
    options.id = "wfrp3-browser";
    options.template = "systems/wfrp3/templates/browser/browser.html"
    options.classes.push("wfrp3", "wfrp-browser");
  ...
  getData() {
    ...
    data.talentMax = wfrp3.talentMax;
    data.trappingTypes = wfrp3.trappingTypes;
    data.lores = this.lores;
    data.characteristics = wfrp3.characteristicsAbbrev;
  ...
  addItems(itemList)
  {
    ...
    this.lores = this.lores.map (p => {
      if (wfrp3.magicLores[p])
        return wfrp3.magicLores[p];
      else
        return p;
    })
    ...
    for (let filter in this.filters.dynamic)
    {
      if (this.filters.dynamic[filter].show && this.filters.dynamic[filter].value)
      {
        switch(filter)
        {
          ...
          case "melee":
            case "ranged":
            filteredItems = filteredItems.filter(i => i.type != "weapon" || filter == wfrp3.groupToType[i.data.data.weaponGroup.value])
            break;
          case "weaponRange":
...
Hooks.on("renderCompendiumDirectory", (app, html, data) => {
  if (game.user.isGM || game.settings.get("wfrp3", "playerBrowser"))
  {
...
Hooks.on('init', () => {
  if (!game.wfrpbrowser)
    game.wfrpbrowser = new Browserwfrp3();
})