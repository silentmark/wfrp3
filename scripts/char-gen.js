...
class Generatorwfrp3 
{
  ...
  static speciesStage()
  {
    renderTemplate("systems/wfrp3/templates/chat/chargen/species-select.html", {species : wfrp3.species}).then(html => {
  ...
  static rollSpecies(messageId, chosenSpecies = null)
  {
    ...
    {
      exp = 0;
      roll = {roll: game.i18n.localize("Choose"), value : chosenSpecies, name : wfrp3.species[chosenSpecies], exp : 0}
    }
    else
    {
      exp = 20;
    ...
    let updateCardData = {roll : roll, species : wfrp3.species}

    // Update the species selection menu to show what was rolled/chosen
    renderTemplate("systems/wfrp3/templates/chat/chargen/species-select.html", updateCardData).then(html =>{
  ...
  static rollAttributes(species, exp = 0, reroll = false)
  {
    ...
    let dataTransfer = {
      ...
      type : "attributes",
      payload : {
        species: wfrp3.species[species],
        characteristics : characteristics,
        movement : wfrp3.speciesMovement[species],
        fate : wfrp3.speciesFate[species],
        resilience : wfrp3.speciesRes[species],
        exp : calcExp
      }
    }
    ...
    for (let abrev in wfrp3.characteristicsAbbrev)
    {
      cardData.characteristics[wfrp3.characteristicsAbbrev[abrev]] = dataTransfer.payload.characteristics[abrev]
    }
    cardData.speciesKey = species;
    cardData.extra = wfrp3.speciesExtra[species]
    cardData.move = wfrp3.speciesMovement[species]

    renderTemplate("systems/wfrp3/templates/chat/chargen/attributes.html", cardData).then(html => {
      let chatData = WFRP_Utility.chatDataSetup(html)
  ...
  static speciesSkillsTalents(species, exp)
  {
    let cardData = {
      speciesKey : species,
      species : wfrp3.species[species],
      speciesSkills : wfrp3.speciesSkills[species],
      exp : exp
    }
    ...
    wfrp3.speciesTalents[species].forEach(talent => {
        if (isNaN(talent))
        {
    ...
    let randomTalents = wfrp3.speciesTalents[species][wfrp3.speciesTalents[species].length-1]
    ...
    renderTemplate("systems/wfrp3/templates/chat/chargen/species-skills-talents.html", cardData).then(html =>{
      let chatData = WFRP_Utility.chatDataSetup(html)
      ChatMessage.create(chatData);
    })
  }
  ...
  {
    let pack = game.packs.find(p => p.collection == "wfrp3.careers")
    ...
    let cardData = {
      ...
      chosen : isChosen,
      speciesKey: species,
      trappings : wfrp3.classTrappings[WFRP_Utility.matchClosest(wfrp3.classTrappings, careerFound.data.data.class.value)] // Match closest is needed here (Academics/Academic)
    }

    // Show card with instructions and button
    renderTemplate("systems/wfrp3/templates/chat/chargen/career-select.html", cardData).then(html => {
      let chatData = WFRP_Utility.chatDataSetup(html)
      ChatMessage.create(chatData);
    })
  }
  ...
  static async rollDetails(species)
  {
    ...
    age = new Roll(wfrp3.speciesAge[species]).roll().total;
    heightRoll = new Roll(wfrp3.speciesHeight[species].die).roll().total;
    hFeet = wfrp3.speciesHeight[species].feet;
    hInches = wfrp3.speciesHeight[species].inches + heightRoll;
    ...
    renderTemplate(`systems/wfrp3/templates/chat/chargen/details.html`, cardData).then(html => {
      let chatData = WFRP_Utility.chatDataSetup(html)
      chatData["flags.transfer"] = JSON.stringify(dataTransfer);
      ChatMessage.create(chatData);
    })
