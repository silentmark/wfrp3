...
class DiceWFRP
{
  ...
  })
  {
    ...
    if (game.settings.get("wfrp3", "testDefaultDifficulty") && (game.combat != null))
      sceneStress = game.combat.started ? "challenging" : "average";
    else if (game.settings.get("wfrp3", "testDefaultDifficulty"))
      sceneStress = "average";
    ...
    let advantageBonus = game.settings.get("wfrp3", "autoFillAdvantage") ? (dialogOptions.data.advantage * 10 || 0) : 0

    mergeObject(dialogOptions.data,
    {
      testDifficulty: dialogOptions.data.testDifficulty || sceneStress,
      difficultyLabels: wfrp3.difficultyLabels,
  ...
  static rollTest(testData)
  {
    ...
    else if (roll.total <= 5 || roll.total <= targetNum)
    {
      ...
      if (game.settings.get("wfrp3", "fastSL"))
      {
      ...
      if (game.settings.get("wfrp3", "testAbove100"))
      {
        if (targetNum > 100)
        {
  ...
  static rollCastTest(testData)
  {
    ...
    if (game.settings.get("wfrp3", "partialChannelling"))
    {
  ...
  static rollChannellTest(testData, actor)
  {
    ...
    if (testResults.description.includes(game.i18n.localize("Failure")))
    {
      ...
      if (Number(SL) == 0 && game.settings.get("wfrp3", "extendedTests"))
        SL = -1;
  ...
  static async renderRollCard(chatOptions, testData, rerenderMessage)
  {
    ...
    if (game.settings.get("wfrp3", "manualChatCards") && !rerenderMessage)
    ...
    if (!rerenderMessage)
    {
      ...
      return renderTemplate(chatOptions.template, chatData).then(html =>
      {
        ...
        if (game.settings.get("wfrp3", "manualChatCards"))
        {
          let blank = $(html)
        ...
        if (chatOptions.sound)
          console.log(`wfrp3 | Playing Sound: ${chatOptions.sound}`)
  ...
  static async chatListeners(html)
  {
    ...
    html.on('change', '.card-edit', ev =>
    {
      ...
      Actorwfrp3[`${data.postData.postFunction}`](newTestData, chatOptions, message);
    })
    ...
    html.on("click", '.chargen-button, .chargen-button-nostyle', event =>
    {
      ...
      {
        case "rollSpecies":
          Generatorwfrp3.rollSpecies($(event.currentTarget).parents('.message').attr("data-message-id"))
          break;
        case "rollCareer":
          Generatorwfrp3.rollCareer($(event.currentTarget).attr("data-species"), wfrp3.randomExp.careerRand)
          break;
        case "rerollCareer":
          Generatorwfrp3.rollCareer($(event.currentTarget).attr("data-species"), wfrp3.randomExp.careerReroll, true)
          Generatorwfrp3.rollCareer($(event.currentTarget).attr("data-species"), wfrp3.randomExp.careerReroll, true)
          break;
        case "chooseCareer":
          Generatorwfrp3.chooseCareer($(event.currentTarget).attr("data-species"))
          break;
        case "rollSpeciesSkillsTalents":
          Generatorwfrp3.speciesSkillsTalents($(event.currentTarget).attr("data-species"))
          break;
        case "rollDetails":
          Generatorwfrp3.rollDetails($(event.currentTarget).attr("data-species"))
          break;

        case "rerollAttributes":
          Generatorwfrp3.rollAttributes($(event.currentTarget).attr("data-species"), Number($(event.currentTarget).attr("data-exp")), true)
          break;
      }
    });
    ...
    html.on("click", '.career-select', event =>
    {
      ...
      Generatorwfrp3.displayCareer(careerSelected, species, 0, false, true)
    });
    ...
    html.on("click", '.market-button', event =>
    {
      ...
      switch ($(event.currentTarget).attr("data-button"))
      {
        case "rollAvailability":
          Marketwfrp3.generateSettlementChoice($(event.currentTarget).attr("data-rarity"));
          break;
        case "payItem":
          if(!game.user.isGM)
          {
            ...
            money = Marketwfrp3.payCommand($(event.currentTarget).attr("data-pay"), money);
            if(money)
            {
        ...
        case "creditItem":
          if(!game.user.isGM)
          {
            ...
            money = Marketwfrp3.creditCommand(dataExchange, money);
            if(money)
            {
        ...
        case "rollAvailabilityTest":
          ...
          Marketwfrp3.testForAvailability(options);
          break;
      }
    });
  }
