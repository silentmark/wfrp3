/**
 * The Itemwfrp3 class only provides two core functionalities
 * 
...
class Itemwfrp3 extends Item
{
  ...
  _mutationExpandData()
  {
    const data = duplicate(this.data.data);
    data.properties = [];
    data.properties.push(wfrp3.mutationTypes[this.data.data.mutationType.value]);
  ...
  _cardExpandData()
  {
    ...
    data.symbols = true;
    data.description = {};
    data.description.value = `<div style='background-color: #2dff0799; width: 100%;
    display: inline-flex;
    flex: 1;
    flex-grow: 1;
    flex-direction: row;'>`;
    let dif = '<span style="color: black;font-family: WFRP3 Symbols;">';
    for(let i = 0; i < data.conservative.difficulty.misfortune; i++) {
      dif += '=';
    }
    dif += '</span><span style="color: purple;font-family: WFRP3 Symbols;">';
    for(let i = 0; i < data.conservative.difficulty.challenge; i++) {
      dif += '<';
    }
    "</span>";
    data.description.value += `<div style="display: flex;">${data.type}</div><div style="display: flex;
    padding-left: 10px;">${dif}</div><div style="display: flex;
    flex-flow: column; width: 100%; text-align: right;">${data.conservative.recharge.value} / ${data.conservative.recharge.max}</div></div>`;
    data.description.value += `<div style='text-align: center; width: 100%;background-color: #2dff0799;'><i>${data.conservative.traits}</i></div>`;
    data.description.value += `<div style='text-align: center; width: 100%;background-color: saddlebrown;color: white;'>${data.conservative.test}</div>`;
    data.description.value += `<div style='background-color: dark-brown;text-align: center; width: 100%;background-color: saddlebrown;color: white;font-family: WFRP3 Symbols;'>${data.conservative.requirements}</div>`;
    data.description.value += data.conservative.description.value;
    data.description.value += "<hr>";

    data.description.value += `<div style='background-color: #ff000099; width: 100%;
    display: inline-flex;
    flex: 1;
    flex-grow: 1;
    flex-direction: row;'>`;
    dif = '<span style="color: black;font-family: WFRP3 Symbols;">';
    for(let i = 0; i < data.reckless.difficulty.misfortune; i++) {
      dif += '=';
    }
    dif += '</span><span style="color: purple;font-family: WFRP3 Symbols;">';
    for(let i = 0; i < data.reckless.difficulty.challenge; i++) {
      dif += '<';
    }
    "</span>";
    data.description.value += `<div style="display: flex;">${data.type}</div><div style="display: flex;
    padding-left: 10px;">${dif}</div><div style="display: flex;
    flex-flow: column; width: 100%; text-align: right;">${data.reckless.recharge.value} / ${data.reckless.recharge.max}</div></div>`;
    data.description.value += `<div style='text-align: center; width: 100%;background-color: #ff000099;'><i>${data.reckless.traits}</i></div>`;
    data.description.value += `<div style='text-align: center; width: 100%;background-color: saddlebrown;color: white;'>${data.reckless.test}</div>`;
    data.description.value += `<div style='background-color: dark-brown;text-align: center; width: 100%;background-color: saddlebrown;color: white;font-family: WFRP3 Symbols;'>${data.reckless.requirements}</div>`;
    data.description.value += data.reckless.description.value;
    data.description.value += "<hr>";
  ...
  _careerExpandData()
  {
    ...
    data.properties.push(wfrp3.statusTiers[this.data.data.status.tier] + " " + this.data.data.status.standing);
    data.properties.push(`<b>${game.i18n.localize("Characteristics")}</b>: ${this.data.data.characteristics.map(i => i = " " + wfrp3.characteristicsAbbrev[i])}`);
  ...
  _weaponExpandData()
  {
    ...
    if (data.weaponGroup.value)
      properties.push(wfrp3.weaponGroups[data.weaponGroup.value]);
    if (data.range.value)
    ...
    if (data.reach.value)
      properties.push(`${game.i18n.localize("Reach")}: ${wfrp3.weaponReaches[data.reach.value] + " - " + wfrp3.reachDescription[data.reach.value]}`);
  ...
  _ammunitionExpandData()
  {
    const data = duplicate(this.data.data);
    let properties = [];
    properties.push(wfrp3.ammunitionGroups[data.ammunitionType.value])
  ...
  postItem()
  {
    ...
    renderTemplate('systems/wfrp3/templates/chat/post-item.html', chatData).then(html =>
    {
  ...
  _trappingChatData()
  {
    const data = duplicate(this.data.data);
    let properties = [
    `<b>${game.i18n.localize("ITEM.TrappingType")}</b>: ${wfrp3.trappingCategories[data.trappingType.value]}`,
    ...
    `<b>${game.i18n.localize("Availability")}</b>: ${wfrp3.availability[data.availability.value]}`
    ]
    return properties;
  }
  ...
  {
    let properties = [
      `<b>${game.i18n.localize("ITEM.MutationType")}</b>: ${wfrp3.mutationTypes[this.data.data.mutationType.value]}`,
    ];
    if (this.data.data.modifier.value)
  ...
  _talentChatData()
  {
    this.data.data.symbols = true;
    const data = duplicate(this.data.data);
    
    let properties = [];
    properties.push(`<b>Card Type: </b> ${data.type}`);
    properties.push(`<b>Attached: </b> ${data.attached.value}`);
    if (data.recharge.value || data.recharge.max) {
      properties.push(`<b>Recharge: </b> ${data.recharge.value} / ${data.recharge.max}`);
    }
    return properties;
  }
  ...
  _careerChatData()
  {
    ...
    properties.push(`<b>${game.i18n.localize("Status")}</b>: ${wfrp3.statusTiers[this.data.data.status.tier] + " " + this.data.data.status.standing}`);
    properties.push(`<b>${game.i18n.localize("Characteristics")}</b>: ${this.data.data.characteristics.map(i => i = " " + wfrp3.characteristicsAbbrev[i])}`);
  ...
  _spellChatData()
  {
    const data = duplicate(this.data.data);
    let properties = [];
    if (wfrp3.magicLores[data.lore.value])
      properties.push(`<b>${game.i18n.localize("Lore")}</b>: ${wfrp3.magicLores[data.lore.value]}`);
    else
  ...
  _weaponChatData()
  {
    ...
    let properties = [
     ...
     if (data.weaponGroup.value)
      properties.push(`<b>Group</b>: ${wfrp3.weaponGroups[data.weaponGroup.value]}`);
    if (data.range.value)
      properties.push(`<b>${game.i18n.localize("Range")}</b>: ${data.range.value}`);
    if (data.damage.value)
      properties.push(`<b>${game.i18n.localize("Damage")}</b>: ${data.damage.value}`);
    ...
    if (data.reach.value)
      properties.push(`<b>${game.i18n.localize("Reach")}</b>: ${wfrp3.weaponReaches[data.reach.value] + " - " + wfrp3.reachDescription[data.reach.value]}`);
    if (data.weaponDamage)
      properties.push(`<b>${game.i18n.localize("ITEM.WeaponDamaged")} ${data.weaponDamage} points</b>`)
    if (data.APdamage)
      properties.push(`${game.i18n.localize("ITEM.ShieldDamaged")} ${data.APdamage} points`)
  ...
  _armourChatData()
  {
    ...
    if (data.armorType.value)
      properties.push(`<b>${game.i18n.localize("ITEM.ArmourType")}</b>: ${wfrp3.armorTypes[data.armorType.value]}`);
    if (data.penalty.value)
      properties.push(`<b>${game.i18n.localize("Penalty")}</b>: ${data.penalty.value}`);
  ...
  _ammunitionChatData()
  {
    const data = duplicate(this.data.data);
    let properties = [];
    properties.push(`<b>${game.i18n.localize("ITEM.AmmunitionType")}:</b> ${wfrp3.ammunitionGroups[data.ammunitionType.value]}`)
...
// Assign Itemwfrp3 class to CONFIG
CONFIG.Item.entityClass = Itemwfrp3;